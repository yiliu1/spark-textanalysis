...
def tokenize(string):
    ...
    """
    wordlist=filter(None,re.split(split_regex,string.lower()))
        
   
    wordfilter=list(filter((lambda i:i not in stopwords),wordlist) )      
...
def countTokens(vendorRDD):
   ...
   """
    Tokens=vendorRDD.map(lambda (x,y):len(y)).reduce(lambda x,y :x+y)
...
def findBiggestRecord(vendorRDD):
    ...
    """
    takens=vendorRDD.map(lambda (x,y):(x,y,len(y))).sortBy(lambda x:-x[2]).collect()
...
print 'There are %s unique tokens in the small datasets.' % uniqueTokenCount ,idfsSmall.takeOrdered(2,lambda s:s[1])


# In[17]:

print idfsSmall.take(1),amazonRecToToken.take(1)
...
print amazonRecToToken.filter(lambda x: x[0] == 'b000hkgj8k').collect()[0]
...
nm = norm(testVec1)
cm=cossim({'photoshop': 22.22222222222222, 'adobe': 8.333333333333334},{'adobe': 8.333333333333334, 'illustrator': 50.0})
print dp, nm, cm
...
k1=tfidf(tokenize('Adobe Photoshop'),idfsSmallWeights)
print k1
k2=tfidf(tokenize('Adobe Illustrator'),idfsSmallWeights)
print k2
k3= cossim(k1,k2)
print k3
...
print similaritiesBroadcast.take(1),type(idfsSmallWeights)
...
sc.parallelize([(2,('m','a')), (3,('a','b')), (4,(1,'b'))]).count()
print 'i am a human'+'yi'
...
trueDupsRDD = (sims.join(goldStandard)).map(lambda (x,(y,z)):(x,y))
trueDupsCount = trueDupsRDD.count()
avgSimDups = (trueDupsRDD.map(lambda (x,y):y).reduce(add)/(float(trueDupsCount)))
nonDupsRDD = (sims.subtract(trueDupsRDD).map(lambda (x,y):y))
              
avgSimNon = nonDupsRDD.reduce(add)/(float(nonDupsRDD.count()))
...
print amazonRecToToken.take(1),idfs(amazonRecToToken).take(1)
...
fullCorpusRDD = amazontoken.union(googletoken)
...
amazonWeightsRDD = amazontoken.map(lambda (x,y):(x,tfidf(y,idfsFullBroadcast.value)))
googleWeightsRDD = googletoken.map(lambda (x,y):(x,tfidf(y,idfsFullBroadcast.value)))
...
def swap1(record1):
    ...
    for i in a :
        for j in b:
            listm.append(((i,j),token1))
        
    return (listm)


commonTokens = (amazonInvPairsRDD.groupByKey().join(googleInvPairsRDD.groupByKey())
                .flatMap(lambda x:swap1(x)).groupByKey()
                .cache())
...
def fastCosineSimilarity(record):
    ...
    s=0
    for i in tokens :
        s=s+(amazonWeightsBroadcast.value[amazonRec][i]*googleWeightsBroadcast.value[googleRec][i])
